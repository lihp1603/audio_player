...
CContex::~CContex()
{
	...
	if(m_pCatFrame){
		free(m_pCatFrame->pData[0]);
		m_pCatFrame->pData[0]=NULL;
		free(m_pCatFrame);
		m_pCatFrame=NULL;
	}
}
    ...
    do{
		...
		if(streamType==MEDIA_TYPE_VIDEO){
			...
			if(m_pCatFrame==NULL){
				...
				catFrame->struAudioParam.nChannelLayout=av_get_default_channel_layout(m_pDecFrame->channels);
				catFrame->struAudioParam.nChannels = av_get_channel_layout_nb_channels(m_pDecFrame->channel_layout);
				...
				frameSize= av_samples_get_buffer_size(NULL, catFrame->struAudioParam.nChannels, catFrame->struAudioParam.nNbSamples, catFrame->struAudioParam.nASFmt, 1);
				...
				memset(catFrame->pData[0],0,frameSize);
